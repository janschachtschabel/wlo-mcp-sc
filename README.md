...
| `WLO_WIDGET_DOMAIN` | unset | all | App identity domain for the ChatGPT plugin submission (required + unique per app there; widgets render under `<domain>.web-sandbox.oaiusercontent.com`). When set, it is emitted as the STANDARD `_meta.ui.domain` **and** the `openai/widgetDomain` alias; when unset, only the alias (edu-sharing origin) is emitted — Claude's MCP-Apps host validates `ui.domain` against its own sandbox format and rejects widgets carrying foreign values. Leave unset for Claude; set only for a ChatGPT submission. The widget CSP allowlist stays the edu-sharing origin regardless. |
