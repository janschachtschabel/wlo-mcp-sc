...
| `WLO_WIDGET_DOMAIN` | edu-sharing origin | all | App identity domain advertised as `_meta.ui.domain` (`openai/widgetDomain`) on the widget resources — required and unique per app at ChatGPT plugin submission; ChatGPT renders widgets under `<domain>.web-sandbox.oaiusercontent.com`. Set it when the app identity differs from the data host (e.g. `https://mcp.wirlernenonline.de`). The widget CSP allowlist stays the edu-sharing origin regardless. |
