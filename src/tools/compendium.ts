...
export function registerCompendiumTool(server: McpServer, readingWidgetUri?: string): void {
  registerWloTool(server, {
    ...
    description: `Hole den redaktionellen Kompendiumstext (kuratierte Übersichts-Prosa) einer oder mehrerer WLO-Sammlungen. Nutze dies, wenn ein Sammlungsergebnis nur einen gekürzten "Kompendium: …"-Auszug zeigt, oder um zusammenzufassen, worum es in einer Sammlung geht. Jede Antwort beginnt mit dem Inhaltsverzeichnis der Überschriften. Mit "query" (z. B. "Lehrplan Thüringen Regelschule") kommen nur die dazu passenden Absätze zurück — deutlich kürzer und die bevorzugte Form, wenn du eine bestimmte Frage hast; ohne "query" der ganze Text, je Hauptabschnitt gekürzt. Gib eine "Sammlung-nodeId" (oder mehrere). NICHT für Dateien/Materialien — gilt nur für Sammlungen mit redaktionellem Text.`,
    inputSchema: {
