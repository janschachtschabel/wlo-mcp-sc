...
| Stateless server | ✅ | one server+transport per request; no session state. The 25–26 read tools all carry `readOnlyHint:true`. **Not read-only as a whole:** the server also lists 14 curation tools, which refuse at call time without a write identity — see the row below |
...
| Least privilege / write scope | ✅ | Anonymous callers get a read-only surface in effect: the 14 curation tools are listed for every caller (since 2026-08-05 — a tool a model never sees is a login that never starts) and each **refuses at call time** without a write-capable identity, carrying the `WWW-Authenticate` challenge that asks the host to offer a login. Every mutation is two-step — preview, then a single-use token bound to a fingerprint of exactly that change — and is read back from the repository before anything is reported. Writes under the shared service account are off unless the operator sets `WLO_ALLOW_SERVICE_WRITES`. Container runs as non-root |
