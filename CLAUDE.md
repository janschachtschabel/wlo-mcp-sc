...
- **Skill-Registry pro Inhaltssammlung — CODE COMPLETE (P0–P3, 2026-08-10);
  ...
  **Superseded 2026-08-12:** the editorial team filed one. `loadSkillRegistry`
  on the Optik collection returned "Skillkatalog Physik Optik" (16 717 chars,
  **28** `:::` blocks — the 56 counted here was the number of FENCE LINES, an
  ...
  code. **Those 28 are gone (2026-08-18, the user's decision):** the same
  document is now 985 characters with **3** blocks, restructured into contexts.
  Do not treat the 28 as a target or a regression — what the measurement proved
  is the PATH, and that still holds. One of the three answers HTTP 403 (not
  publicly readable), so the live document is also the first real case of
  `unresolved` being non-empty.
...
- **Schreibziel Original + erweiterte Metadatenfläche — VOLLSTÄNDIG (P0–P5,
  ...
  **P3 hat (b) gemessen, und das Ergebnis streicht P4 zusammen (2026-08-17).**
  ...
  3. **Übrig bleiben drei Felder statt dreier Feldgruppen**, alle vokabular-rein
     ...
     entfällt, **T4.3 (Schreibfläche) entfällt**. — **Die zweite Hälfte davon ist
     am 2026-08-18 aufgehoben (Nutzerentscheidung):** die 14 Felder sind ZWEI
     Familien, und die Erhebung hat sie als eine behandelt. Die fünf
     BEFUND-Felder (`correctness`, `copyright_law`, `criminal_law`,
     `personal_law`, `protection_of_minors`) deklarieren EIN vollständig
     beschriftetes Vokabular, das Maschine von Mensch unterscheidet
     (`auto_findings` „Auffälligkeiten gefunden (Maschine)" …), und vier der
     fünf werden im Korpus bereits so benutzt (52/97, 54/98, 50/92). Sie sind
     seither schreibbar — Vokabular `qualityFinding` in `vocabs.ts`, Felder in
     `WRITABLE_FIELDS`, Parameter `qualityCorrectness` … an
     `wlo_create_content`/`wlo_update_content`. Zwei Regeln binden das: die
     beiden MENSCH-Werte werden VERWEIGERT (der Wert benennt, wer geprüft hat,
     und ein Modell kann nicht bezeugen, dass eine Person hingesehen hat), und
     die sieben STERNE-Felder bleiben geschlossen (redaktionelles Urteil, und
     der Korpus speichert dort eine Ziffer, wo das Widget eine URI deklariert).
     **Am 2026-08-19 kamen die sieben 0–5-SKALEN dazu** (didactics, language,
     medial, neutralness, transparentness, data_privacy, currentness), lesend
     wie schreibend. Drei Messungen binden das: (a) der Korpus speichert BEIDE
     Formen im selben Feld (`…/quality_didactics/1` neben nacktem `"4"`), und nur
     die URI-Form kommt mit `_DISPLAYNAME` zurück — die Ziffer ist kein kaputter
     Wert, sondern dieselbe Position, weshalb `src/vocabs-quality-scale.ts`
     (ERZEUGT aus dem Metadatensatz, `scripts/generate-quality-scales.mjs`) sie
     beschriftet; (b) geschrieben wird die DEKLARIERTE Form, und die ist je Feld
     verschieden — URI bei sechs Skalen, nackte Ziffer bei `currentness`; (c)
     `login` und `relevancy_for_education` bleiben zu (0–1, echte Ja/Nein-Fragen)
     und `ccm:containsAdvertisement` ebenfalls (0–5 deklariert, aber 69 628 von
     69 688 Werten sind `yes`/`no` — ein Stern dort wäre die dritte Schreibweise).
     Gemessen ist auch, warum das Lesen überhaupt geht: `containsAdvertisement`
     `5` heißt **„✰✰✰✰✰ ohne Werbung"**, eine nackte „5" sagt das Gegenteil.

     **Die Bestätigungsvorschau beschriftet seit dem 2026-08-19 jeden
     Vokabularwert** (`displayValue`, `services/write/change-set.ts`): vorher
     stand dort die rohe URI, und der Bestätigungsschlüssel bindet an genau
     diesen Satz — dieselbe Regel wie `nameOf` bei den Themenseiten. `PREVIEW_VOCAB`
     ist bewusst NICHT `FIELD_VOCAB`: das eine entscheidet, was geschrieben werden
     darf, das andere nur, was ein Satz sagt.

     Der Rest des Absatzes gilt unverändert. `ccm:oeh_quality_login` ist das
...
- **Kontexte in der Skill-Registry — VOLLSTÄNDIG (P0–P6, 2026-08-18):**
  ...
  **Die Anleitungsebenen bleiben getrennt und benannt (2026-08-18).**
  `contextInstructions` liefert `{scope, title, text}` je Ebene (allgemein ·
  Elternkontext · Kontext), und `subjectRegistryText` rendert **eine Zeile je
  Ebene** mit einer Beschriftung davor (`instructionLabel`, `tools/shared.ts`).
  Vorher wurden sie mit einem Leerzeichen verkettet, und ein Leser konnte nicht
  sehen, wo der allgemeine Teil endet und der Kontext beginnt — die Redaktion
  schreibt sie als getrennte Abschnitte, weil sie Verschiedenes bedeuten. Zwei
  Regeln binden das: **`flattenText` läuft je TEIL, nie über den
  zusammengesetzten Block** (es kollabiert jeden Whitespace-Lauf, Zeilenumbrüche
  eingeschlossen — über dem Block angewandt macht es die Trennung sofort wieder
  rückgängig), und die Kappung bleibt EINE über dem ganzen Block, damit das
  Budget dasselbe ist wie vorher. Der Kontextname geht durch `sanitizeText`,
  nicht `oneLine`: er landet in Prosa, die eine Person befolgen soll.

  **Jede Sammlungs-Antwort trägt die Beschreibungen der ersten DREI Skills
  (2026-08-18, Nutzerentscheidung — zuerst nur am benannten Kontext, am selben Tag
  auf beide Pfade erweitert).** Eine Stelle rendert es (`skillDescriptionLines`,
  `tools/shared.ts`), weil beide Pfade es brauchen und eine zweite Kopie still den
  Deckel verlöre. `describeEntries` (`services/skill-registry.ts`) läuft **nach**
  dem Verengen — nicht `loadSkillRegistry({resolveHeads:true})`, das jeden
  deklarierten auflöst (bis `REGISTRY_MAX` = 100) und für eine verengte Antwort
  die falsche Rechnung stellt. Fünf Regeln binden das. (1) **`DESCRIBED_MAX` = 3
  ist ein Deckel auf die ABRUFE, nicht nur auf die Ausgabe** — das ist der ganze
  Grund, warum es bezahlbar ist; wer ihn hebt, hebt die Abrufe je
  Sammlungs-Antwort mit. Der Katalog, der mit SUCHTREFFERN reist, ist ein anderer
  Weg (Cache, bis zu 10 Sammlungen je Anfrage) und bleibt kostenlos — dorthin
  gehört die Prosa weiterhin NICHT. (2) Schlagworte bleiben draußen (gemessen ~175 Zeichen je
  Skill gegen ~170 für die Beschreibung). (3) Die Beschreibungen stehen
  **unterhalb** der Liste, nicht darin: die Liste ist auf `REGISTRY_INLINE_MAX`
  Zeilen gedeckelt, und je eine Zeile mehr pro Skill kippt einen Vier-Skill-Kontext
  in die Kurzform ohne nodeIds — Information hinzuzufügen darf die Antwort nicht
  ihre brauchbare Hälfte kosten. (4) Eingerückt wird **nach** dem Kappen:
  `capText` trimmt, also gehen vorher eingebaute Führungszeichen genau auf der
  ERSTEN Zeile verloren, was wie eine Überschrift mit Unterliste aussieht. Und die
  Kopfzeile bekommt dafür einen vierten `reach`-Fall (`opts.described`): wo die
  Beschreibungen schon dastehen, darf sie nicht auf `get_skill_registry` „für
  Beschreibungen" verweisen, sondern nennt Schlagworte und das Dokument.
  (5) Ein Skill, dessen Kopf sich nicht lesen ließ, kommt als `unreadable`
  zurück und wird als „Nicht abrufbar (geprüft für die ersten N)" benannt statt
  mit „laden mit get_skill" angeboten — wir haben den Abruf bezahlt, also WISSEN
  wir es. Der Satz nennt seine eigene Reichweite, weil der Deckel bedeutet, dass
  die übrigen ungeprüft sind; ohne diese Angabe liest sich ein Befund über drei
  von zehn wie ein Urteil über alle zehn.

  **Der Katalog hängt VOLLSTÄNDIG an jeder Sammlungs-Antwort (2026-08-18,
  Nutzerentscheidung).** Skills + erste drei Beschreibungen + die **allgemeine
  Anweisung** der Redaktion. Drei Regeln. (1) Die Anweisung liegt auf
  `CacheEntry.generalInstruction`, **niemals** auf `CachedRegistry` — das ist der
  Typ, den ein Suchtreffer-Knoten trägt, und Prosa dort landet in jedem Treffer.
  Gemessen ist das auch der Grund gegen den naheliegenden Live-Pfad: warme
  Übersicht 286–393 ms gegen ~1 851 ms mit Dokument-Lesen. (2) Sie ist beim
  SPEICHERN gedeckelt (`GENERAL_INSTRUCTION_MAX` = 1200), nicht beim Rendern —
  der Cache hält bis zu `CACHE_MAX_ENTRIES` = 2000 Einträge, ungedeckelt wäre die
  Eintragsgrenze keine Speichergrenze mehr. (3) Ein TREFFER behält **seinen
  eigenen** Kontext in der Gliederung (`contexts.filter(c => c.path === …)`), damit
  die Gruppierung Kontext-Skills von den immer geltenden trennt — wie
  `get_skill_registry` es tut. Das frühere `contexts: []` war der Fix für einen
  ANDEREN Fehler (die Zahl des Dokuments über den Einträgen eines Kontexts); ein
  einzelner Kontext mit seiner eigenen Zahl hat ihn nicht. Darunter stehen die
  übrigen Kontextnamen, damit ein zweiter, genauerer `get_skill_registry`-Aufruf
  ohne Zwischenschritt möglich ist.

  Zwei Sätze waren dabei falsch und sind korrigiert: „in 1 Kontexten" (eine
  Registry mit genau einem Kontext ist normal) und die Kontextzahl auf einer
  verengten Antwort — die war die der SICHT in einem Satz, der wie eine Aussage
  über die Registry liest. Eine verengte Antwort nennt jetzt gar keine Zahl;
  ihren Kontext nennt sie in der Eingangszeile.
...
- **Vokabular-Abgleich: was eine Beschriftung trägt — VOLLSTÄNDIG (2026-08-18):**
  ...
  **Ein Wert, den niemand beschriften kann, wird WEGGELASSEN (2026-08-18).**
  Nachgemessen am Datensatz `7affb314-3f66-4a86-955d-161239ec63b2`:
  `ccm:containsAdvertisement = ["5"]` mit **leerem** `_DISPLAYNAME` — die
  Sterne-Skala des Widgets statt des yes/no-Vokabulars, das 69 628 von 69 688
  Werten tragen. Der Slug-Rückfall druckte daraus `Werbung: 5`. Neben
  `Kosten: nein` und `Zugang: ohne Anmeldung` liest sich das wie eine
  beschriftete Aussage, und gerade die RICHTUNG ist nicht rekonstruierbar — auf
  dem einen Feld, wo sie verkehrt herum aus „werbefrei" „voller Werbung" macht.
  `labels()` verwirft deshalb einen Slug, der nur aus Ziffern besteht. Das ist
  dieselbe Regel, aus der 11 der 14 Qualitätsfelder gar nicht gelesen werden,
  angewandt je WERT statt je Feld — ein sauberes Feld hält gelegentlich einen
  schmutzigen. Gemessen kostet es nichts, was gebraucht wird: über 119
  Datensätze 63 Wort-Werte gegen **1** nackte Zahl, und `ccm:license_oer` (2
  Träger, numerische Slugs `oer/1`, `oer/2`) war in **beiden** Fällen
  beschriftet.
...
- **Kompendiumstext: Inhaltsverzeichnis + gezielte Passagen — VOLLSTÄNDIG
  (2026-08-18):** `docs/plans/2026-08-18-kompendium-bm25-design.md` (Entwurf und
  Aufgaben in einer Datei).

  `get_compendium_text` nimmt einen `query`. Mit ihm kommen die passenden
  Absätze zurück (BM25, `src/text-bm25.ts`), ohne ihn der ganze Text mit je
  Hauptabschnitt gedeckelten Abschnitten. Das **Inhaltsverzeichnis geht in
  beiden Fällen mit** — ein Modell, das nur Ausschnitte sieht, weiß sonst nicht,
  was es nicht gesehen hat, und kann die zweite, genauere Frage nicht stellen.

  Vier Messungen binden jede Änderung hier (Staging, 2026-08-18, 158 Sammlungen
  über acht Suchwörter, davon **11 mit Kompendiumstext** — neu messen, bevor man
  ihnen widerspricht):

  1. **„je H1 kappen" kappt das ganze Dokument.** 10 von 10 Texten mit
     Überschriften tragen **genau eine H1** (den Titel) und 11–18 H2. Der Deckel
     gilt deshalb je **Hauptabschnitt**, und das ist die flachste
     Überschriftenebene, die **mindestens zweimal** vorkommt (`topLevelOf`,
     `services/compendium-view.ts`) — sonst die flachste überhaupt, ohne
     Überschrift der ganze Text. Bei 2 000 ändert sich genau **ein** Text
     (Optik, 65 250 → 18 744 Zeichen Körper, 6 von 12 Abschnitten), die anderen
     neun kommen unverändert. Der Deckel ist für den Ausreißer.
  2. **Ein roher Absatz ist die falsche BM25-Einheit.** 329 der 972 echten
     Absätze sind kürzer als 40 Zeichen (Tabellenzeilen, Listenpunkte); BM25
     normalisiert auf die Länge, also schlägt die 4-Zeichen-Zeile den Absatz,
     der sie erklärt. Eine Passage ist ein **Absatzlauf innerhalb EINES
     Abschnitts, aufgesammelt bis 200 Zeichen** — und sie überschreitet nie eine
     Überschrift, sonst trüge sie einen Pfad, den ihr Text nicht stützt.
  3. **Der Abschnitt als Einheit wäre zu grob:** p50 276 Zeichen, aber der
     größte Eigentext hat **16 317**.
  4. **Nicht getroffene Suchwörter werden benannt.** „Lehrplan Thüringen
     Regelschule" trifft auf Optik nur über `lehrplan`; die Antwort ist voller
     Lehrpläne aus Rheinland-Pfalz. `rankBm25` liefert `unmatched` deshalb selbst
     — ein Aufrufer, der es herleitet, braucht eine zweite Kopie der Trefferregel.

  Fünf Regeln der Umsetzung. (a) Die **Termhäufigkeit zählt über `termMatches`**,
  nicht über Token-Gleichheit: deutsche Komposita sind der gemessene Grund, aus
  dem `node-match.ts` existiert. **Dazu gehört zwingend (a2): die ANFRAGE geht
  durch denselben Tokenizer wie die Dokumente** (`normalizedTerms`,
  `text-bm25.ts`). `queryTerms` allein trennt nur an Leerraum, lässt also
  Satzzeichen am Wort — und sobald die Dokumentseite Token für Token verglichen
  wird, trifft so ein Term gar nichts: gemessen kam „Rheinland-Pfalz" als NICHT
  GEFUNDEN zurück über einen Text, der es siebenmal nennt, und eine natürliche
  Frage verlor ihr letztes Wort ans „?". Das fälscht genau die Zeile, die dieses
  Paket als Ehrlichkeits-Zusage führt. `queryTerms` behält dabei die Hoheit
  darüber, WELCHE Wörter zählen, damit die Stoppwortregel eine Heimat hat; ein
  `Set` darüber, weil ein zweimal gesagtes Wort sonst doppelt punktet und
  zweimal in `unmatched` steht. (b) Serverseitiges (Titel, Inhaltsverzeichnis,
  Hinweiszeile) steht **vor** dem ungeprüften Dokument — dieselbe Regel wie bei
  `get_skill`; jede daraus stammende Zeile geht durch `oneLine`. Eine Überschrift
  mit `\r` kann die Gliederung trotzdem nicht fälschen, und zwar aus einem
  anderen Grund als vermutet: JavaScripts `.` schließt `\r` aus, die Zeile gilt
  gar nicht erst als Überschrift (gemessen). (c) **Kein Treffer ist kein Fehler**
  und fällt NICHT auf den Volltext zurück — die Antwort ist dann das
  Inhaltsverzeichnis plus der Satz, dass nichts passte. (d) Im JSON trägt eine
  `query`-Antwort `passages` und **kein** `compendiumText`: beides nebeneinander
  hebt die Verengung auf, um die der Aufrufer gebeten hat. `getCompendiumTexts`
  bleibt unangetastet — `services/search.ts` und `/api/compendium` hängen daran.

  `WLO_COMPENDIUM_SECTION_MAX` (Standard 2000) ist eine **Betreiber**-Einstellung
  und kein Aufrufparameter: der Deckel existiert, damit eine Antwort nicht endlos
  wird, und wer ihn hochsetzen kann, hat keinen.
