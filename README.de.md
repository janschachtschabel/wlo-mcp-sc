...
| `WLO_WIDGET_DOMAIN` | edu-sharing-Origin | alle | App-Identitäts-Domain, die als `_meta.ui.domain` (`openai/widgetDomain`) an den Widget-Ressourcen ausgewiesen wird — bei der ChatGPT-Plugin-Submission Pflicht und pro App eindeutig; ChatGPT rendert Widgets unter `<domain>.web-sandbox.oaiusercontent.com`. Setzen, wenn die App-Identität vom Daten-Host abweicht (z. B. `https://mcp.wirlernenonline.de`). Die Widget-CSP bleibt unabhängig davon auf der edu-sharing-Origin. |
