...
| `WLO_WIDGET_DOMAIN` | nicht gesetzt | alle | App-Identitäts-Domain für die ChatGPT-Plugin-Submission (dort Pflicht + pro App eindeutig; Widgets rendern unter `<domain>.web-sandbox.oaiusercontent.com`). Wenn gesetzt, wird sie auf `_meta.ui.domain` **und** dem Alias `openai/widgetDomain` ausgewiesen; **wenn nicht gesetzt, auf keinem von beiden** — ein Host validiert die Domain gegen sein eigenes Sandbox-Format und verwirft bei fremden Werten das ganze Widget (und bricht den zugehörigen Tool-Call ab): Claude erwartet `{hash}.claudemcpcontent.com` und normalisiert den Vendor-Alias auf den Standard-Key, es reicht also nicht, nur einen wegzulassen. Für Claude und jeden Nicht-ChatGPT-Host nicht setzen. Die *Verbindungs*-Liste der Widget-CSP bleibt unabhängig davon auf der edu-sharing-Origin — zur Bildliste siehe die nächste Zeile. |
| `WLO_WIDGET_IMAGE_DOMAINS` | `https://img.youtube.com` | alle | Zusätzliche Origins, von denen die Widget-CSP **Vorschaubilder** laden darf, kommagetrennt. Erweitert ausschließlich `resource_domains`; `connect_domains` bleibt allein das Repository, denn das Widget stellt selbst keine Anfragen. Die Vorgabe ist gemessen, nicht geraten: das Repository liefert die Vorschau nicht immer selbst — bei YouTube-Datensätzen antwortet `/preview` mit `302` auf den Thumbnail-Host des Anbieters (2026-08-21: 3 von 78 auflösbaren Vorschauen), und ein Browser prüft die CSP-Host-Regel bei **jeder** Weiterleitungsstufe erneut. Ohne den Eintrag zeigten diese Kacheln ein kaputtes Bild, während die Web-Oberfläche des Repositorys das Thumbnail zeigt. **Datenschutz:** ein freigegebener Host erhält die IP-Adresse der betrachtenden Person, sobald eine solche Kachel gerendert wird — mit dem Wert `none` wird das vollständig unterbunden (die Kacheln zeigen dann ihr Dokumentsymbol). Es muss dieses Wort sein und kein leerer Wert: `docker-compose.yml` reicht jede Einstellung als `"${VAR:-}"` durch, ein nicht konfigurierter Container zeigt also immer einen Leerstring. |
...
- **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** — die Entwicklerkarte: Schichten, Nahtstellen, Invarianten und die gemessenen Randbedingungen, die eine Änderung einhalten muss (auf Englisch).
